    ...
    public void onInitializeClient() {
        ...
        ClientPlayConnectionEvents.JOIN.register((handler, sender, client) -> {
            BuiltInRegistries.ITEM.forEach(item->{
                var trimMaterial = item.getDefaultInstance().get(DataComponents.PROVIDES_TRIM_MATERIAL);
                if (trimMaterial!=null && trimMaterial.material().key().isPresent()) {
                    Identifier sprite = Identifier.withDefaultNamespace("item/"+BuiltInRegistries.ITEM.getKey(item).getPath());
                    TooltipsGlobals.TRIM_MATERIALS.put(trimMaterial.material().key().get().identifier().toString(), sprite);
                }
            });
        });
    }
